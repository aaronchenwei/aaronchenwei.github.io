...
function asyncThunkWithPromise() {
  return (dispatch, getState) => {
    ...
    fetch("https://<host>:<port>/<api>/<entity>")
      ...
      .then(json => dispatch({ type: "json", name: "posts", value: json }))
      .catch(e => dispatch({ type: "error", name: "error", value: e.message }));
...
function asyncThunkWithAwait() {
  return async (dispatch, getState) => {
    ...
    try {
      const response = await fetch("https://<host>:<port>/<api>/<entity>");
...
function asyncThunkWithYield() {
  return async (dispatch, getState) => {
    co(function*() {
      ...
      try {
        const response = yield fetch("https://<host>:<port>/<api>/<entity>");
